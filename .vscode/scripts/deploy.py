...
def launch_sims(targets):
    for t in targets:
        ...
        if sim:
            ...
            if subprocess_conout is not None:
                try:
                    out = subprocess_conout.subprocess_conout(sim, nrows=9999, encode=True)
                    print(out)
                    continue
                except Exception as e:
                    print(f"[SIM] Console capture unavailable ({type(e).__name__}: {e}).")

            # Non-Windows fallback: launch simulator without CONOUT capture.
            try:
                if isinstance(sim, str):
                    subprocess.Popen(sim, shell=True)
                else:
                    subprocess.Popen(sim)
            except Exception as e:
                print(f"[SIM] Failed to launch simulator: {e}")
